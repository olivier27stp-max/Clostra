    ...
    <div className="flex min-h-screen items-center justify-center bg-[#F5F6F8] px-4">
      <div className="w-full max-w-md">
        ...
        <div className="mb-6 flex flex-col items-center">
          <ClostraLogo size={36} />
          <h1 className="mt-4 text-lg font-semibold text-text-primary">{t('onboarding_title')}</h1>
          <p className="mt-1 text-sm text-text-tertiary">{t('onboarding_subtitle')}</p>
        </div>
          ...
          {Array.from({ length: TOTAL_STEPS }, (_, i) => (
            <div key={i} className="flex-1">
              <div className={cn(
                ...
                i + 1 <= step ? 'bg-brand' : 'bg-border-subtle'
              )} />
            </div>
          ))}
        </div>
        <p className="mb-4 text-center text-[11px] text-text-muted">
          {t('step_x_of_y', { step, total: TOTAL_STEPS })}
        </p>
        ...
        <div className="rounded-xl border border-border-subtle bg-white p-6 shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-text-secondary">{t('first_name')}</label>
                  <Input
                    placeholder="Jean"
                    value={firstName}
                ...
                <div className="space-y-1.5">
                  <label className="block text-sm font-medium text-text-secondary">{t('last_name')}</label>
                  <Input
                    placeholder="Dupont"
                    value={lastName}
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-text-secondary">{t('company_name')}</label>
                <Input
              ...
              <div className="space-y-1.5">
                <label className="block text-sm font-medium text-text-secondary">{t('role')}</label>
            ...
            <div className="space-y-4">
              <div className="space-y-1.5">
                ...
                <textarea
                  ...
                  rows={4}
                  className="w-full rounded-lg border border-border-subtle bg-white px-3 py-2 text-sm text-text-primary placeholder:text-text-muted transition-colors hover:border-border focus:border-brand focus:outline-none focus:ring-2 focus:ring-brand/15 resize-none"
                />
              </div>
              ...
              <div className="rounded-lg border border-border-subtle bg-surface-elevated p-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-text-muted">{t(`role_${role}` as any)} &middot; {companyName}</p>
          ...
          <div className="mt-6 flex gap-3">
            ...
            <Button
              ...
              onClick={handleNext}
            >
              {step === TOTAL_STEPS ? t('finish_setup') : t('continue')}
            </Button>
          </div>
        </div>
      </div>
