...
import { LumeCreateModal } from './lume-create-modal';
...
const SALES_REPS = [
  { id: 'rep-1', name: 'Sarah Chen' },
  { id: 'rep-2', name: 'Marcus Davis' },
  { id: 'rep-3', name: 'Emily Rodriguez' },
  { id: 'rep-4', name: 'Jordan Kim' },
];
...
export function MapContainer() {
  ...
  // --- Lume ---
  const [lumeCreatePin, setLumeCreatePin] = useState<LeadPinData | null>(null);
  const [lumeDetailPin, setLumeDetailPin] = useState<LeadPinData | null>(null);
  ...
  function placePin(map: mapboxgl.Map, pin: LeadPinData) {
    ...
    function refreshPopupHTML() {
      ...
      popup.setHTML(createLeadPinPopupHTML(current, editId, delId, lumeId));
    }
    ...
    popup.on('open', () => {
      ...
      setTimeout(() => {
        ...
        document.getElementById(lumeId)?.addEventListener('click', () => {
          const current = markersRef.current.get(pin.id)?.pin || pin;
          popup.remove();
          if (current.lume_status === 'created' && current.lume_customer_id) {
            setLumeDetailPin(current);
          } else {
            setLumeCreatePin(current);
          }
        });
      }, 0);
    });
    ...
    function initMap(center: [number, number], startZoom: number) {
      ...
      map.on('load', () => {
        ...
        getCachedMapPins<LeadPinData>().then((saved) => {
          if (saved?.length) saved.forEach((pin) => placePin(map, pin));
        }).catch(() => {});

        // Load saved zones
        getCachedMapZones<ZoneData>().then((saved) => {
          if (saved?.length) { zonesRef.current = saved; renderZonesOnMap(); bump(); }
        }).catch(() => {});
        ...
        map.on('click', (e) => {
          ...
          if (containerRef.current?.dataset.mapMode === 'add_pin') {
            ...
            const pin: LeadPinData = { id, lat, lng, status, name: 'Nouveau lead', address: `${lat.toFixed(5)}, ${lng.toFixed(5)}`, note: '' };
            placePin(map, pin);
            ...
            fetch(`https://api.mapbox.com/geocoding/v5/mapbox.places/${lng},${lat}.json?access_token=${token}&language=fr&limit=1`)
              ...
              .then((data) => {
                ...
                const rec = markersRef.current.get(id);
                if (rec) { rec.pin.name = place?.text || 'Nouveau lead'; rec.pin.address = place?.place_name || pin.address; savePins(); }
              }).catch(() => {});
            return;
          }
  ...
  function handleSaveEdit() {
    ...
    const wasClosedWon = editingPin.status === 'closed_won';
    const isNowClosedWon = editStatus === 'closed_won';
    removePin(editingPin.id);
    const updated: LeadPinData = { ...editingPin, name: editName, status: editStatus, note: editNote };
    placePin(mapRef.current, updated);
    setEditingPin(null);
    if (isNowClosedWon && !wasClosedWon && !updated.lume_customer_id) setLumeCreatePin(updated);
  }

  // ---------------------------------------------------------------------------
  // Lume handlers
  // ---------------------------------------------------------------------------
  function handleLumeSuccess(pinId: string, response: LumeCreateResponse) {
    const rec = markersRef.current.get(pinId);
    if (rec) { rec.pin.lume_customer_id = response.customer_id; rec.pin.lume_job_id = response.job_id; rec.pin.lume_status = 'created'; rec.pin.lume_error = null; }
    setLumeCreatePin(null);
  }
  function handleLumeSkip(pinId: string) {
    const rec = markersRef.current.get(pinId);
    if (rec) rec.pin.lume_status = 'not_created';
    setLumeCreatePin(null);
  }
  ...
  return (
    ...
    >
        ...
        <div className="flex h-full w-full items-center justify-center bg-[#080b10]">
          <div className="max-w-md rounded-2xl border border-white/8 bg-white/[.03] p-8 text-center">
            ...
            <div className="mt-4 rounded-lg border border-white/8 bg-black/40 p-3 text-left">
              <code className="text-xs text-indigo-300">NEXT_PUBLIC_MAPBOX_TOKEN=pk.eyJ1Ijo...</code>
            </div>
          </div>
        </div>
      )}
                    ...
                    {statuses.map(([key, cfg]) => {
                      ...
                      return (
                        <button key={key} onClick={() => toggleFilter(key)}
                          className={`flex w-full items-center gap-2.5 rounded-lg px-3 py-1.5 text-left transition-all ${isActive ? 'bg-white/8 text-white' : 'text-white/30 hover:text-white/50'}`}
                        >
      ...
      {/* Lume modals */}
      {lumeCreatePin && (
        <LumeCreateModal pin={lumeCreatePin} onSuccess={(response) => handleLumeSuccess(lumeCreatePin.id, response)} onSkip={() => handleLumeSkip(lumeCreatePin.id)} onClose={() => setLumeCreatePin(null)} />
      )}
      {lumeDetailPin && lumeDetailPin.lume_customer_id && (
        <LumeDetailPanel customerId={lumeDetailPin.lume_customer_id} jobId={lumeDetailPin.lume_job_id || null} onClose={() => setLumeDetailPin(null)} onRetry={() => { setLumeDetailPin(null); setLumeCreatePin(lumeDetailPin); }} />
      )}
    </div>
  );
}
